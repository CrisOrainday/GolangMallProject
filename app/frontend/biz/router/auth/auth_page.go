...
func Register(r *server.Hertz) {
	...
	root := r.Group("/", rootMw()...)
	{
		...
		_auth.POST("/login-register-emailcode", append(_loginregisterwithemailcodeMw(), auth.LoginRegisterWithEmailCode)...)
		_auth.POST("/logout", append(_logoutMw(), auth.Logout)...)
		_auth.POST("/register", append(_registerMw(), auth.Register)...)
		_auth.POST("/send-email-code", append(_sendemailcodeMw(), auth.SendEmailCode)...)
	}
}
